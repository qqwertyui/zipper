...
Zip::Zip(const char *filename) {
    ...
    if(this->lfhs.size() != this->cdfhs.size() ) {
        // We assume that number of lfs must be the same as the number of cdfh, if it is not we
        // assume it is broken
        return;
    }
	this->good = true;
...
bool Zip::read_cdfhs(unsigned char *data, ECDR *ecdr, std::vector<CDFH*> &cdfhs) {
	...
	for(size_t i=0; i<ecdr->number_entries; i++) {
		...
		ptr += sizeof(CDFH_base) + t->name_length + t->extra_length + t->comment_length;
	}
	return true;
}

bool Zip::read_lfhs(unsigned char *data, std::vector<CDFH*> &cdfhs, std::vector<LFH*> &lfhs) {
	...
	if(entry->c_method == 0) {
		if(entry->crc32 == 0) {
			if(entry->c_size == 0 && entry->uc_size == 0) {
				if(entry->name_length > 0 && entry->name[entry->name_length-1] == '/') {
					return true;
				}
			}
		}
...
void Zip::list_files(enum Flag f) {
	...
	puts("----------------------------------\n");
    for(vector_cdfh::iterator i=this->cdfhs.begin(); i!=this->cdfhs.end(); i++) {
		CDFH *t = *i;
...
unsigned int Zip::extract_all(enum Flag flag) {
	for(vector_cdfh::iterator i=this->cdfhs.begin(); i!=this->cdfhs.end(); i++) {
		CDFH *cdfh = *i;
		char *name = Utils::copy_string((const char*)cdfh->name, cdfh->name_length);
