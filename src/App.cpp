...
int main(int argc, char **argv) {
    ...
    // argc and argv are modified below to make it possible to put the first argument without dash
	char c;
	...
	if(operation == Zip::Job::LIST) {
		...
		unsigned int status = Misc::is_wildcard_present(target) ? z1.extract_all((Zip::Flag)flag) : z1.extract(target, (Zip::Flag)flag);
		if(status == Status::OK) {
